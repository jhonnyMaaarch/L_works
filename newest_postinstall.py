...
### Project: RHEL post install. Mainly for OS version using pythonV2            ###
###          For OS versions using python v3 Please use the twin -              ###
###         newest_postinstall_v8.py - found in /u/sunny/Tools or xcatmn3:      ###
########################################################################
########################################################################
import os
import subprocess
...
def main():
   ...
   # shutil.copy('resolv.conf', resolv.conf+'_orig') 
    ...
    os.system('ifconfig')
    net_int = raw_input('Enter the network interface to be configured; ')
    ipADDR= raw_input('provide ip address (e.g 192.168.7.70): ')
    netMASK = '255.255.0.0'
