...
#    vm_dir = raw_input("Enter a vm directory to create one or use an existing: ")
...
#     print("You now have a vm directory: "+ vm_dir)
    ...
    img_list = {'RH7.6':['RHEL-7.6-20181010.0-Server-x86_64-dvd1.iso'], 'RH7.7':['RHEL-7.7-20190723.1-Server-x86_64-dvd1.iso'], 'RH7.9':['RHEL-7.9-20200917.0-Server-x86_64-dvd1.iso'], 'RH8.0':['RHEL-8.0.0-20190404.2-x86_64-dvd1.iso'], 'RH8.1':['RHEL-8.1.0-20191015.0-x86_64-dvd1.iso'], 'RH8.2':['RHEL-8.2.0-20200404.0-x86_64-dvd1.iso'], 'SL15':['SLE-15-Installer-DVD-x86_64-GM-DVD1.iso'], 'SL15.1':['SLE-15-SP1-Installer-DVD-x86_64-GM-DVD1.iso'], 'SL15.2':['SLE-15-SP2-Full-x86_64-GM-Media1.iso'], 'UB16':['ubuntu-16.04.6-server-amd64.iso']}
    ...
    if img_Ask == 'y':
     ...
     subprocess.call('scp c84f1u01:/root/power_kvm/'+iso_file + ' ./', shell=True)
    else:
     iso_file = raw_input('Copy from the above list of iso files and paste here (e.g RHEL-7.6-20181010.0-Server-x86_64-dvd1.iso): ')
    subprocess.call('scp c84f1u01:/root/os_list '+'./', shell=True)
    n_arch = subprocess.check_output('uname -m', shell=True).rstrip()
    print(n_arch)
    node_arch = raw_input('Enter above architecture (e.g x86_64): ')
    rawfile_name = my_os+'.'+node_arch+'.raw'
    disk_name = my_os+'.'+node_arch+'.qcow2'
    raw_file = subprocess.check_output('qemu-img convert -O raw '+iso_file+' '+rawfile_name, shell=True)
    img_file = subprocess.check_output('qemu-img convert -O raw '+rawfile_name+' '+disk_name, shell=True)
    subprocess.call('chmod +x '+disk_name, shell=True)
    ...
    ram_size = raw_input('Provide your desired RAM size (e.g 4096): ')
    os.system('pwd')
    os.system('ls')
    ...
    network_path = 'http://10.28.0.30:80/install/'
    ...
    subprocess.call('virt-install ' + '--name '+ vm_name + ' --ram ' + ram_size + ' --disk path=' + disk_path + ',size=' + disk_size + ' --vcpus ' + Vcpus + ' --os-type ' + Os_type + ' --os-variant ' + os_Variant + ' --location ' + network_path + os_Variant + '/' + node_arch + ' --network bridge='+vnet_int + ' --extra-args ' + '"ip='+vm_ipAddr+'::10.28.0.1:255.255.240.0:'+vm_name+'.pokprv.stglabs.ibm.com'+':'+nic_int+':none"', shell=True) 
