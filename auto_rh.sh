...
echo "would you like to install errata kernel:(y or n)"
read err_kernel
    if [ "err_kernel" = "y" ]
    then
#setup errata repository
        cd
        read -p  "Enter OS version (e.g rh7.6, rh7.7 or rh8.1):" os_ver
        if [[ $os_ver == rh7 ]]; then
          scp xcatmn3:/u/sunny/ibm-rhsm.sh ./
          chmod +x ibm-rhsm.sh
          ./ibm-rhsm.sh
        elif [[ $os_ver == rh8 ]]; then
          scp xcatmn3:/u/sunny/ibm-yum-rhel8.sh ./
          chmod +x ibm-yum-rhel8.sh
          ./ibm-yum-rhel8.sh
        else
